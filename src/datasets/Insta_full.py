...
def insta_full_load_data(ds_path='../../ds/Instacart/full_insta_df.csv', debug=False):
    ...
    G_user = {uidx: set() for uidx in range(user_num)} # {uidx: [tidx,]}
    G_item = {tidx: set() for tidx in range(item_num)} # {tidx: [uidx,]}

    print(f'now constructing the bipartite graph (time consuming)')
